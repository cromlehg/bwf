...
class SlickAccountDAO @Inject()(val dbConfigProvider: DatabaseConfigProvider,
	...
	def _size = table.size.result.map(_.toLong)
	...
	def _createAccountWithRoles(login: String, email: String, roles: Seq[String]) =
		_createAccount(login, email) flatMap { account =>
			roleDAO._assignAndReturnRolesNameToTargetIfNotAssigned(roles, account.id, RoleTargetTypes.ACCOUNT) map { roles =>
				account.copy(roles = roles.toList)
			}
		}
	...
	override def createAccountWithRoles(login: String, email: String, roles: Seq[String]): Future[Account] =
		db.run(_createAccountWithRoles(login, email, roles).transactionally)
